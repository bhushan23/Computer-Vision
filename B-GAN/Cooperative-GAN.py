    ...
    for epoch in range(num_epochs):
        for data in data_loader:
            ...
            for each in Generators:
                ...
                D_fake1 = D(G_fake1)
                # Compute Generator Loss
                G_loss1 = lossCriterion(D_fake1, Variable(torch.ones(batch_size, 1)))
    ...
    for epoch in range(num_epochs):
        for data in data_loader:
            ...
            for each, each_opt in zip(Generators, GOptimizers):
                ...
                lossList.append(float(G_loss.data[0]))
                D.zero_grad()
                each.zero_grad()
                G_loss.backward()
                each_opt.step()
            ...
            #print type(lossList[0])
        BestPerformingGenerator = lossList.index(min(lossList))
        print 'BEFORE COPY: Epoch [{}/{}], Discriminator {:.4f}, Best Generator {:.4f}'.format(epoch+1, num_epochs, D_loss.data[0], GeneratorLoss[BestPerformingGenerator].data[0])
        ...
        #print epoch
        print 'Epoch [{}/{}], Discriminator {:.4f}, Best Generator {:.4f}'.format(epoch+1, num_epochs, D_loss.data[0], GeneratorLoss[BestPerformingGenerator].data[0])
